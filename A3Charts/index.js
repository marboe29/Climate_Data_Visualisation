...
function drawLolly() {
  ...
  var margin = { top: 25, right: 30, bottom: 50, left: 100 },
  ...
  //Adds X axis that extends max chart width by x axis hjighest value
  var x = d3
    .scaleLinear()
    //.domain([0, 8000])
  ...
  svg
    .append("text")
    .attr("transform", "rotate(-90)")
    .attr("y", 15 - margin.left + 30)
    .attr("x", 0 - (height / 2 - 125))
    .style("font-size", "10px")
    .attr("dy", ".25em")
    .style("text-anchor", "middle")
    .text("2010-2015 period");
  ...
  function update(selectedVar) {
    ...
    */
      ...
      j.enter()
        ...
        .transition()
        .duration(1000)
        .attr("x1", function (d) {console.log(x(d[selectedVar]));return x(d[selectedVar]);})
        .attr("x2", x(0))
        .attr("y1", function (d) {return y(d.City);})
        .attr("y2", function (d) {return y(d.City);})
        .attr("stroke", "grey");

      //variable u maps data to existing circle circles -> start at X=0
  ...
  d3.csv("data.csv").then(function (data) {
    d3.csv("datapresent.csv").then(function (dataTwo) {
      ...
      svg
        ...
        .style("text-transform", "uppercase")
        .style("font-weight", "bold")
        .text("Mean monthly air temperature (°C)");
...
function drawheatMap() {
  ...
  legend
    .append("rect")
    .attr("x", function (d, i) {
      return legendElementWidth * i + 100;
    })
  ...
  legend
    ...
    .attr("x", function (d, i) {
      return legendElementWidth * i + 125;
    })
